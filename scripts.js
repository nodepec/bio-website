document.addEventListener("DOMContentLoaded", () => {
    ...
    const createDynamicBackground = () => {
        ...
        class Particle {
            constructor() {
                ...
                this.size = Math.random() * 3 + 0.5; // Reduced size for subtler effect
                this.speedX = (Math.random() * 0.5 - 0.25) * 0.5; // Reduced speed
                this.speedY = (Math.random() * 0.5 - 0.25) * 0.5; // Reduced speed
                this.color = 'rgba(255, 255, 255, 0.7)'; // White color with some transparency
            }
            ...
            update() {
                ...
                if (this.size > 0.1) this.size -= 0.02; // Slower shrinking
        ...
        const animateParticles = () => {
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                for (let j = i; j < particles.length; j++) {
                    ...
                    if (distance < 100) {
                        ctx.beginPath();
                        ctx.strokeStyle = 'rgba(255, 255, 255, 0.1)'; // Very subtle white connections
                        ctx.lineWidth = 0.1;
            ...
            if (particles.length < 50) { // Maintain fewer particles
                createParticles();
            }
        ...
        canvas.addEventListener('mousemove', (event) => {
            for (let i = 0; i < 2; i++) { // Reduced number of particles on mouse move
                particles.push(new Particle());
    ...
    window.addEventListener("load", () => {
        ...
        // Hide spinner with fade out and zoom effect
        setTimeout(() => {
